...
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get('DATABASE_URL', 'sqlite:///reading_tracker_demo.db')
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
app.config['GOOGLE_CLIENT_ID'] = os.environ.get('GOOGLE_CLIENT_ID', 'demo-client-id')
...
@app.route('/login')
def login():
    return render_template('login_demo.html', demo_mode=DEMO_MODE)

@app.route('/demo-login', methods=['POST'])
def demo_login():
    """Demo login endpoint that bypasses Google OAuth"""
    if not DEMO_MODE:
        return jsonify({'error': 'Demo mode not enabled'}), 400
...
def google_auth():
    """Original Google OAuth endpoint (disabled in demo mode)"""
    if DEMO_MODE:
        return jsonify({'error': 'Use demo login in demo mode'}), 400
    
    # Original Google OAuth code would go here
    return jsonify({'error': 'Google OAuth not configured in demo'}), 400
